...
// ── Faculty Modal ──────────────────────────────────────────────────────────────
function FacultyModal({ open, initial, onClose }: { open: boolean; initial?: Faculty; onClose: () => void }) {
...
function RoomTable({ rooms, faculties }: { rooms: RoomOption[]; faculties: Faculty[] }) {
  ...
  const columns = [
    ...
    },
    {
      title: "Loại phòng",
      dataIndex: "roomType",
      render: (v: string) => <Tag color={roomTypeColor(v)}>{roomTypeLabel(v)}</Tag>,
    },
    {
      title: "Trạng thái",
      dataIndex: "status",
      render: (v: string) => (
        <Badge status={v === "ACTIVE" ? "success" : "default"} text={v === "ACTIVE" ? "Hoạt động" : "Ngừng"} />
      ),
    },
    {
      title: "Thao tác",
      ...
      render: (_: unknown, record: RoomOption) => (
        <HasPermission requiredPermissions={["room.manage", "faculty.manage", "system.manage"]}>
          <Space>
  ...
  return (
    <div>
      ...
      <Table
        ...
        bordered={false}
      />
      <RoomModal open={!!editRoom} initial={editRoom} faculties={faculties} onClose={() => setEditRoom(undefined)} />
      <RoomModal open={addRoom} faculties={faculties} onClose={() => setAddRoom(false)} />
    </div>
  );
}
...
export const FacultyRoomTab = () => {
  ...
  return (
    <div>
      ...
      {isError && <Alert type="error" message="Không thể tải danh sách khoa & phòng" showIcon className="mb-4" />}
